...
function makeMove(playerId, matchId, x, y) {
  ...
  if (match.winner) return { error: "Game over" };

  const playerSymbol = match.players[0] === playerId ? "X" : "O";
  if (playerSymbol !== match.currentPlayer) return { error: "Not your turn" };
  if (match.board[x][y]) return { error: "Cell already occupied" };
